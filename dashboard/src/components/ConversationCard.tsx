...
export default function ConversationCard({
  ...
  const accentDark = TYPE_AVATAR_DARK[info.type] || "#475569";

  // Use color-mix-like rgba for badge backgrounds
    ...
    >
      <div
        className={`
          ...
          ${expanded ? "shadow-lg border-[#cbd5e1] ring-1 ring-[#e2e8f0]" : "shadow-sm hover:scale-[1.005]"}
        `}
      >
        {/* Card header */}
        <div
        ...
        >
          ...
          <div className="flex-1 p-4 flex items-start gap-4">
            {/* Avatar / icon */}
            <div
              className="w-10 h-10 rounded-full shrink-0 flex items-center justify-center text-white text-sm font-bold shadow-sm"
              style={{
                background: `linear-gradient(135deg, ${accentColor}, ${accentDark})`,
              }}
            >
              {TYPE_AVATAR_LETTER[info.type] || "?"}
            </div>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-4 mt-2 flex-wrap">
                <span
                  ...
                  title={formatDateFr(conversation.started_at)}
                >
                  {formatDateSmart(conversation.started_at)}
                </span>
            ...
            {/* Expand chevron */}
            <svg
              className={`w-5 h-5 text-[#cbd5e1] shrink-0 transition-transform duration-200 mt-1 ${
                expanded ? "rotate-180" : ""
              }`}
              fill="none"
              viewBox="0 0 24 24"
