...
- Comporte-toi normalement comme dans le reste de ce prompt`;
  ...
  function connectVoice() {
    console.log("[ARGO] connectVoice() called (proxy WS, clé côté serveur)");
    console.log("[ARGO] Opening WebSocket to proxy:", LIVE_WS_URL);
    const ws = new WebSocket(LIVE_WS_URL);

    ws.onopen = () => {
      console.log("[ARGO] ✅ WebSocket OPEN. Sending setup (prompt+clé injectés côté serveur).");
